...
run_window_lmer <- function(pc, q, epoch, varname, use_next_trial = TRUE, time_window = c(-0.5, 0.5)) {
  library(lme4)
  library(tidyverse)
  library(car)
  library(emmeans)
  ...
  print(paste0('UPDATED PC',pc,' Q',q,': ',epoch,': ',varname, ' next trial=', use_next_trial))
  print(time_window)
  ...
  if (use_next_trial) {
    ...
    if (grepl("err", varname)) {
      df <- df %>%
        filter(.data[[value_col]] < 60) %>%
        filter(.data[[varname]] < 60)
    }
  } else {
    ...
    if (grepl("err", varname)) {
      df <- df %>%
        filter(.data[[value_col]] < 60) %>%
        filter(.data[[paste0("last_", varname)]] < 60)
    }
  }
  ...
  formula <- if (use_next_trial) {
    as.formula(paste0(value_col, " ~ group * mean_burst_count + trial + block + ", 
                      varname, " + (1 + group*mean_burst_count + ", varname, " | subject)"))
  } else {
    as.formula(paste0(value_col, " ~ group * mean_burst_count + trial + block + ", 
                      paste0('last_',varname), " + (1 + group*mean_burst_count + ", 
                      paste0('last_',varname), " | subject)"))
  }
  ...
  out <- list(Anova = res, emtrends = pw)
  return(out)
}
  ...
  for (epoch in c("vis", "mot")) {
    for (pc in 7:10) {
      ...
      for (q in 1:4) {
        ...
        if (use_next_trial) {
          select_vars <- c(select_vars, varname)
        } else {
        ...
        if (use_next_trial) {
          formula <- as.formula(paste0(value_col, " ~ group * burst_count + trial + block + ", 
                                       varname, " + (1 + group*burst_count + ", varname, " | subject)"))
        } else {
          formula <- as.formula(paste0(value_col, " ~ group * burst_count + trial + block + ", 
                                       paste0("last_", varname), " + (1 + group*burst_count + ", 
                                       paste0("last_", varname), " | subject)"))
        }
        ...
        for(cluster_id in cluster_ids) {
          ...
          if(nrow(cluster_df) >= min_cluster_size) {
            ...
            win_results <- run_window_lmer(pc, q, epoch, varname, use_next_trial = use_next_trial, 
                                        time_window = c(start, end))
      ...
      # FIXED: Save files INSIDE pc loop with proper naming
      filename_suffix <- if (use_next_trial) paste0(varname, "_next_trial") else varname
      results <- bind_rows(all_perm_results)
      write_csv(results, paste0("output/PC_", pc, "_", epoch, "_", filename_suffix, "_permutation.csv"))
      write_csv(all_win_results, paste0("output/PC_", pc, "_", epoch, "_", filename_suffix, "_windows.csv"))
    }
  }
}
...
run_burst_rate_trend_analysis <- function(varname, use_next_trial = TRUE) {
  ...
  for (epoch in c("vis", "mot")) {
    for (pc in 7:8) {
      ...
      write_csv(results, paste0("output/PC_", pc, "_", epoch, "_", suffix, ".csv"))
    }
  }
}
